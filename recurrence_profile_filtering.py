...
data = {subject : pd.read_csv(f'outputs/recurrence_profiles/{subject}.tsv', sep='\t') for subject in subjects}

# drop variants with NaN values
# ignore patients who have no samples collected beyond baseline

to_filter = []
for subject in subjects:
    data[subject] = lq.drop_nans(data[subject])
    if f'{subject}B' in data[subject].columns:
...
for subject in subjects:
    ...
    data_tree[subject] = lq.filter_coverage(data_tree[subject], 10, [f'{subject}_Buffy', f'{subject}_Core', f'{subject}_Margin'])
    # drop variants with no variant supporting reads in any sample
    data_tree[subject] = lq.filter_supporting_reads(data_tree[subject], 1, [f'{subject}_Buffy', f'{subject}_Core', f'{subject}_Margin'])
    ...
    data_tree[subject] = lq.filter_VAF(data_tree[subject], 5, [f'{subject}_Buffy', f'{subject}_Core', f'{subject}_Margin', 'somatic_candidate'])
    # fit filtering parameters to data
    tempdata = lq.fit_filter(data[subject], data_tree[subject])
    # save filtered data
    tempdata.to_csv(f'outputs/recurrence_profiles/{subject}_filtered.tsv', sep='\t', index=False)
    ...
    tempdata = lq.fit_filter(data[subject], data_tree[subject])
    tempdata.to_csv(f'outputs/recurrence_profiles/{subject}_filtered_HNSCC.tsv', sep='\t', index=False)