...
data = {subject : pd.read_csv(f'outputs/tissue_data/{subject}.tsv', sep='\t') for subject in subjects}
...
for subject in subjects:
    ...
    # save filtered data
    tempdata.to_csv(f'outputs/tissue_data/{subject}_filtered.tsv', sep='\t', index=False)
    ...
    tempdata = lq.fit_filter(data[subject], data_tree[subject])
    tempdata.to_csv(f'outputs/tissue_data/{subject}_filtered_HNSCC.tsv', sep='\t', index=False)